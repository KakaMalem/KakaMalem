...
// Helper to get a usable URL from Payload's `string | Media` shape
...
const fetchHomeData = async (): Promise<HomeData> => {
  try {
    // Fetch products
    const productsRes = await fetch('/api/products?limit=20&depth=1', { cache: 'no-store' })
    const productsData = await productsRes.json()
    const allProducts: Product[] = productsData.docs || []

    // Fetch categories
    const categoriesRes = await fetch('/api/categories?limit=10&depth=1', { cache: 'no-store' })
    const categoriesData = await categoriesRes.json()
    const categories: Category[] = categoriesData.docs || []

    // Split products into different sections
    const heroProducts = allProducts.filter((p) => p.salePrice).slice(0, 3)
    const featuredDeals = allProducts.filter((p) => p.salePrice).slice(0, 4)
    const trendingProducts = allProducts.slice(0, 8)

    return {
      heroProducts: heroProducts.length > 0 ? heroProducts : allProducts.slice(0, 3),
      categories,
      featuredDeals: featuredDeals.length > 0 ? featuredDeals : allProducts.slice(0, 4),
      trendingProducts,
    }
  } catch (error) {
